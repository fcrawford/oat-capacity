...
param.sumry <- function(smpl.joint, city){
   if (city==1){Bs0 <- 829
               Bp0 <- 0}
   if (city==2){Bs0 <- 478
               Bp0 <- 30}
...
nms <- c("lam", "lam0", "lam1", "alpha.f", "alpha0.f", "alpha1.f", "irr.alpha", "k.al",
                           "delta", "gamma", "rho.n", "rho0.n", "rho.f", "rho0.f", "rho1", 
                           "mu.s", "mu0.s", "mu1.s", "irr.mu", "k.mu", "u", "p", 
                           "m.DU", "m.OAT", "m.A", 
                           "S0", "E0", "On0", "Of0", "Q0", "A0", "N0", "v0", "w0",
...
# contagion = 1 is base case, contagion = 0 is constant rates assumption (all parameters except mu)
...
ode.mod.inputs.means <- function(joint, city, Cs, Cp, ppy, contagion){
## constants ##
if (city==1){
         Bs0 <- 829
         Bp0 <- 0
...
ode.mod.inputs.draw <- function(joint, j, city, Cs, Cp, ppy, contagion){
## constants ##
if (city==1){
         Bs0 <- 829
         Bp0 <- 0
...
out$du.init <- with(out, S*(lam0+lam1*((On+Of+Q)/(N-E))))
return(out)
}
